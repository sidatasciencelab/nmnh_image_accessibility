...
def embed_and_classify_file_batch(file_list, output_file):
    ...
    batch_size = 32
    class_labels = ['A photo of an animal', 
            'A museum specimen']
    embed_list = []
    class_probs = []
    ...
    with open(f'iz_embeddings/{output_file}.npy','wb') as np_file:
        np.save(np_file, embeddings)

    prob_stack = np.vstack(class_probs)
    prob_df = pd.DataFrame(prob_stack)
    prob_df.columns = ['live_photo','specimen']
    prob_df['filename'] = [file.stem for file in file_list]
    prob_df.to_csv(f'iz_probabilities/{output_file}.tsv', sep='\t', index=False)
